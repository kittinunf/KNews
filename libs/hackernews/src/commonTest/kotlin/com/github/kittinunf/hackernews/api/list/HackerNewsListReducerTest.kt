...
class HackerNewsListReducerTest {
    ...
    fun `should bring uiState to Success state when load action is ended with success`() {
        ...
        val state = reducer(currentState, LoadStoriesResult(Result.success(listOf(createRandomStory(1), createRandomStory(2)).map(::listUiRowStateMapper))))
    ...
    fun `should bring uiState to Success state when load next action is ended with success`() {
        ...
        val state = reducer(currentState, LoadNextStoriesResult(Result.success(listOf(createRandomStory(3), createRandomStory(4)).map(::listUiRowStateMapper))))
    ...
    fun `should add the stories into the main stories list after load next action is ended with success`() {
        ...
        val state = reducer(
            currentState,
            LoadNextStoriesResult(Result.success(listOf(createRandomStory(3), createRandomStory(4), createRandomStory(5)).map(::listUiRowStateMapper)))
        )
    ...
    fun `should not do anything with the main stories list after load next action is ended with failure`() {
        ...
        val (_, reducer) = LoadNextStoriesResultReducer()
        val state = reducer(currentState, LoadNextStoriesResult(Result.success(listOf(createRandomStory(1)).map(::listUiRowStateMapper))))
    ...
    fun `should sort according to the current sorting condition`() {
        ...
        val (_, reducer) = LoadStoriesResultReducer()
        val state = reducer(
            currentState, LoadStoriesResult(Result.success(listOf(createRandomStory(1), createRandomStory(2), createRandomStory(4)).map(::listUiRowStateMapper)))
        )
