...
class HackerNewsDetailMapperTest {
    ...
    fun `should map the timeAgo for ListUiRowState correctly`() {
        ...
        assertEquals("10 seconds ago", mapper.map(secondsAgoComment).fromNowText)
        ...
        assertEquals("10 minutes ago", mapper.map(tenMinutesAgoComment).fromNowText)
        ...
        assertEquals("10 hours ago", mapper.map(tenHoursAgoComment).fromNowText)
    }
}
